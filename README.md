...
Run this locally using the following command:
> go build -o userland-names/userland-names ./userland-names && sam local start-api

Then test it with curl as follows:
> curl -X POST -H "Content-Type: application/json" --data "@$(pwd)/test.json" http://localhost:3000/


# userland

This is a sample template for userland - Below is a brief explanation of what we have generated for you:

```bash
.
├── Makefile                    <-- Make to automate build
├── README.md                   <-- This instructions file
├── userland-names              <-- Source code for a lambda function
...
aws cloudformation describe-stacks \
    --stack-name userland --query 'Stacks[].Outputs'
```
